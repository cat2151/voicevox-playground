...
export async function getAudioQuery(
	...
	try {
		...
		if (!response.ok) {
			throw new Error(
				`Audio query failed: ${response.status} ${response.statusText}`,
			);
		}
		...
		if (error instanceof Error && error.name === "AbortError") {
			throw new Error(
				"VOICEVOXサーバーへの接続がタイムアウトしました。サーバーが起動しているか確認してください。",
			);
		}
		throw error;
	} finally {
...
export async function synthesize(
	...
	try {
		...
		if (!response.ok) {
			throw new Error(
				`Synthesis failed: ${response.status} ${response.statusText}`,
			);
		}
