...
export function drawRealtimeFFT(
	...
	const { topFreq, minTopBin, topBinSet } = getTopFreqInfo(
		fftValues,
		minBin,
		binCount,
		nyquist,
	...
	const isBelowThreshold =
		globalFFTMax !== -Infinity && dbDiff > FFT_VOLUME_DB_THRESHOLD;
	...
	if (!isFFTEmpty && !isBelowThreshold) {
		const { peakX, peakY } = findPeakPosition(
			fftValues,
			minTopBin,
			minBin,
			binCount,
...
function getTopFreqInfo(
	...
	const valueWithIndex = Array.from(fftValues, (v, i) => ({ v, i })).filter(
		({ i }) => i >= minBin,
	);
