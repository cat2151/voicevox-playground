...
function mapIntensityToSpectrogramColor(intensity: number) {
	...
	for (let i = 0; i < SPECTROGRAM_COLOR_STOPS.length - 1; i++) {
		...
		if (clamped >= current.stop && clamped <= next.stop) {
			const localT =
				(clamped - current.stop) / Math.max(next.stop - current.stop, 1e-6);
			const [r, g, b] = lerpColor(current.color, next.color, localT);
			return `rgb(${r},${g},${b})`;
		}
	}
	const [r, g, b] =
		SPECTROGRAM_COLOR_STOPS[SPECTROGRAM_COLOR_STOPS.length - 1].color;
	return `rgb(${r},${g},${b})`;
}

function determineSpectrogramCeiling(
	values: Float32Array,
	...
	const rawCeiling = Math.max(
		highestIndexAboveThreshold,
		previousCeiling * 0.98,
	);
	const clampedCeiling = Math.max(
		1,
		Math.min(values.length - 1, Math.floor(rawCeiling)),
	);
	return clampedCeiling;
}

export async function analyzeSpectrogramFrames(
	buffer: AudioBuffer,
	columns: number,
) {
	...
	const processChunk = async (startColumn: number, endColumn: number) => {
		for (let column = startColumn; column < endColumn; column++) {
			...
			const startSample = Math.max(
				0,
				Math.min(Math.floor(position * maxStartSample), maxStartSample),
			);
	...
	const ceilingIndex = determineSpectrogramCeiling(
		peakMagnitudes,
		binCount - 1,
	);
...
export function drawSpectrogram(
	...
	const cappedTargetX = Math.min(
		drawableWidth,
		Math.max(0, Math.floor(progress * drawableWidth)),
	);
		...
		for (let bin = 0; bin < values.length; bin++) {
			...
			const normalized =
				scale === "log"
					? freq <= 0
						? 0
						: (Math.log10(Math.max(freq, minLogFreq)) -
								Math.log10(minLogFreq)) /
							Math.max(Math.log10(maxFreq) - Math.log10(minLogFreq), 1)
					: freq / maxFreq;

			const nextBin = bin + 1;
			const nextFreq =
				nextBin > ceilingIndex ? maxFreq : (nextBin / ceilingIndex) * maxFreq;
			const nextNormalized =
				scale === "log"
					? nextFreq <= 0
						? 0
						: (Math.log10(Math.max(nextFreq, minLogFreq)) -
								Math.log10(minLogFreq)) /
							Math.max(Math.log10(maxFreq) - Math.log10(minLogFreq), 1)
					: nextFreq / maxFreq;
			...
			const yBottom =
				drawableHeight -
				Math.min(nextNormalized * drawableHeight, drawableHeight);
	...
	for (const freq of ticks) {
		const normalized =
			scale === "log"
				? freq <= 0
					? 0
					: (Math.log10(Math.max(freq, minLogFreq)) - logMin) /
						Math.max(logMax - logMin, 1)
				: freq / maxFreq;
		const y =
			drawableHeight - Math.min(normalized * drawableHeight, drawableHeight);
		ctx.beginPath();
...
export function drawOfflineSpectrogram(
	...
	const { ctx, width, height } = prepareCanvas(canvas);
	if (ctx) {
		drawTimeTicks(ctx, data.duration, width, height, {
			leftMargin: SPECTROGRAM_LEFT_MARGIN,
		});
	}
}
