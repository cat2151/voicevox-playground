...
export async function replayCachedIntonationAudio() {
	...
	try {
		...
		const decodedBuffer = await audioContext.decodeAudioData(
			appState.lastSynthesizedBuffer.slice(0),
		);
		if (renderedCanvas) {
			drawRenderedWaveform(decodedBuffer, renderedCanvas);
		}
		playbackResult = await playAudio(
			decodedBuffer,
			realtimeCanvas,
			spectrogramCanvas,
...
export async function fetchAndRenderIntonation(text: string, styleId: number) {
	...
	try {
		...
		state.intonationSelectedIndex =
			state.intonationPoints.length > 0 ? 0 : null;
