...
export function getBaseDisplayRange(): { min: number; max: number } | null {
	...
	const span = Math.max(
		state.intonationInitialPitchRange.max -
			state.intonationInitialPitchRange.min,
		0,
	);
...
export function applyRangeExtra(desiredExtra: RangeExtra) {
	...
	state.intonationRangeExtra = clampRangeExtra(
		desiredExtra,
		baseRange,
		dataRange,
	);
...
export function clampPitchToDisplayRange(pitch: number) {
	...
	return Math.min(
		Math.max(pitch, state.intonationDisplayRange.min),
		state.intonationDisplayRange.max,
	);
}
...
export function calculateLetterKeyAdjustment(params: {
	...
	if (tentativePitch > maxWithExtra) {
		desiredTopExtra = Math.max(
			desiredTopExtra,
			tentativePitch - params.baseRange.max,
		);
	} else if (tentativePitch < minWithExtra) {
		desiredBottomExtra = Math.max(
			desiredBottomExtra,
			params.baseRange.min - tentativePitch,
		);
	}
	...
	return {
		pitch,
		rangeExtra: { top: desiredTopExtra, bottom: desiredBottomExtra },
	};
}
...
export function updateHoveredLabel(index: number | null) {
	...
	const spans =
		state.intonationLabelsEl.querySelectorAll<HTMLElement>(".intonation-label");
	spans.forEach((span) => {
		const spanIndex =
			span.dataset.moraIndex != null ? Number(span.dataset.moraIndex) : -1;
...
export function drawIntonationChart(points: IntonationPoint[]) {
	...
	if (state.intonationMaxValueEl)
		state.intonationMaxValueEl.textContent = `${rangeMax.toFixed(1)}`;
	if (state.intonationMinValueEl)
		state.intonationMinValueEl.textContent = `${rangeMin.toFixed(1)}`;
	...
	const pointSpacing = Math.max(
		1,
		(width - margin * 2) / Math.max(points.length - 1, 1),
	);
	state.intonationPointPositions = points.map((point, index) => {
		const x = margin + index * pointSpacing;
		const normalized =
			(clampPitchToDisplayRange(point.pitch) - rangeMin) / rangeSpan;
...
export function adjustIntonationScale(
	direction: "top" | "bottom",
	factor: number,
) {
	if (direction === "top") {
		state.intonationTopScale = Math.max(
			0.05,
			state.intonationTopScale * factor,
		);
	} else {
		state.intonationBottomScale = Math.max(
			0.05,
			state.intonationBottomScale * factor,
		);
	}
	refreshDisplayRange();
